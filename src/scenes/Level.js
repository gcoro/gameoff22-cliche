...
import { Scp173 } from "./scp173/Scp173"
...
export class Level extends Phaser.Scene {
    ...
    initAlienInteraction(mode) {
        // mode is 'gameOver' 'restart' 'minigame'
    ...
    talkAboutScp() {
        ...
        let scp
        if (!this.lastMinigame)
            scp = items[Math.floor(Math.random() * items.length)]
        ...
        const index = 0
        let discourse =
            scp === Scp173.name ? this.strings.scp173 : this.strings.scp5153
        const part2 =
            scp === Scp173.name ? this.strings.scp173_2 : this.strings.scp5153_2
    ...
    nextLine(discourse, index, scp, gameOver) {
        ...
        if (discourse[index]) {
            if (discourse[index] === this.strings.clicheRight[0]) {
                // exception in size for cliche right
                this.createSpeechBubble(discourse[index], null, null, 200, 100)
            } else if (gameOver) {
                // exception in size for game over text
    ...
    createSpeechBubble(quote, x, y, width, height) {
        ...
        if (!x)
            x =
                this.alienSprite.body.position.x +
                this.alienSprite.body.halfWidth
        if (!y)
            y =
                this.alienSprite.body.position.y -
                this.alienSprite.body.halfHeight -
                height
