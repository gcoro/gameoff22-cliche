...
class Meteor extends Phaser.Scene {
	...
	create() {
		...
		this.showMessage(["Defend the SCP-2000 until it","is fully charged to save humanity,","but do it before the meteor","collides with the Earth!","Every metorite falling on the structure","will compromise it and decrease its charge","","- ARROW/WASD to move","-SPACE BAR to shoot","-SHIFT to boost ","","","Press any key to start game!"])
		const resume = (event) => {
			console.log(event)
			this.hideMessage()
